...
class GearResumeSession extends Transaction.GearTransaction {
    _api;
    constructor(_api) {
        super(_api);
        this._api = _api;
    ...
    init({ programId, allocations, codeHash, }) {
        try {
            this.extrinsic = this._api.tx.gear.resumeSessionInit(programId, Array.from(createType.CreateType.create('BTreeSet<u32>', allocations).toU8a()), codeHash);
            return this.extrinsic;
        }
        catch (error) {
            console.log(error);
            throw new program_errors.ResumeSessionInitError(programId, error.message);
        }
    }
    /**
    ...
    push({ sessionId, memoryPages }) {
        if (!memoryPages.every(([_, page]) => {
            ...
            throw new program_errors.ResumeSessionPushError(sessionId, 'Invalid memory page length. Must be 16KB.');
        }
        const vecLen = createType.CreateType.create('Compact<u8>', memoryPages.length).toHex();
        const tuples = memoryPages.map(([number, page]) => {
            const num = createType.CreateType.create('u32', number).toHex();
            const p = typeof page === 'string' ? page : util.u8aToHex(page);
        ...
        try {
            ...
            throw new program_errors.ResumeSessionPushError(sessionId);
        }
    }
    /**
    ...
    commit({ sessionId, blockCount }) {
        try {
            ...
            console.log(error);
            throw new program_errors.ResumeSessionCommitError(sessionId, error.message);
        }
    }
}

exports.GearResumeSession = GearResumeSession;
